              ...
              child: filtered.isEmpty
                  ...
                  : ListView.separated(
                      ...
                      itemBuilder: (_, index) {
                        ...
                        final items = (discount['items'] as List? ?? [])
                            .cast<Map<String, dynamic>>();
                        final finalAmount = (discount['amount'] as num?)?.toDouble() ?? amount;
                          ...
                          child: ListTile(
                            ...
                            subtitle: Column(
                              ...
                              children: [
                                ...
                                Text(
                                  'Taux: ${rate.toString()}${isPercent ? '%' : ' TND'} • ${_formatCurrency(amount)}',
                                ),
                                Text(
                                  'Sous-total: ${_formatCurrency(subtotal)} • ${_formatDate(timestamp)}',
                                ),
                              ],
                            ),
                            trailing: Row(
      ...
      builder: (context) => AlertDialog(
        title: Row(
          children: [
            ...
            const SizedBox(width: 8),
            const Text('Ticket de remise'),
          ],
        ),
        content: SizedBox(
          width: MediaQuery.of(context).size.width * 0.5,
          child: SingleChildScrollView(
            child: PaymentTicketCard(
              table: table,
              server: server,
              noteName: noteName,
              timestamp: timestamp,
